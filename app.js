...
let MONGODB_URL = 'mongodb+srv://amrutashirahattimath:UWeJfixl49fIAIRr@cluster0.8kl0pu2.mongodb.net/?retryWrites=true&w=majority'
...
app.post('/login',async(req,res)=>{
    try{
        ...
        let user = await User.findOne({email});
         if(!user){
            return res.status(400).json({
                message: "user is not registered,pleas register first"
            })
        }
...
//task 10 craeate route to delete product in cart
app.delete('/cart/product/delete',async(req,res)=>{
    try{
        ...
        //findindex() searxhes for product in cart
